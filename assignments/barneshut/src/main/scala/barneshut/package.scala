...
package object barneshut {
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = {
      if (size <= minimumSize)
        ...
        newBodies.foreach(b => newFork = newFork.insert(b))
        newFork
      }
    }
  }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      def stripNumber(value: Int, lowerBound: Int, upperBound: Int): Int =
        if (value < lowerBound) lowerBound
        else if (value > upperBound) upperBound
        else value

      val x: Int = stripNumber(((b.x - boundaries.minX) / sectorSize).toInt, 0, sectorPrecision - 1)
      val y: Int = stripNumber(((b.y - boundaries.minY) / sectorSize).toInt, 0, sectorPrecision - 1)
      apply(x, y) += b
      this
    }
